...
async function main() {
    try {
        ...
        const buildSearchUrl = (page = 1) => {
            ...
            if (minBedrooms) url.searchParams.set('beds_min', String(minBedrooms));
            if (maxBedrooms) url.searchParams.set('beds_max', String(maxBedrooms));
...
async function main() {
    try {
        ...
        const extractPropertyLinks = ($) => {
            ...
            $('a[href*="/plp/buy/"], a[href*="/plp/rent/"]').each((_, el) => {
                const href = $(el).attr('href');
                if (href && (href.includes('/plp/buy/') || href.includes('/plp/rent/'))) {
                    const absUrl = toAbsoluteUrl(href);
                    if (absUrl && absUrl.match(/-(for-sale|for-rent)-/)) {
                        links.add(absUrl);
                    }
                }
            });
         ...
         * Extract basic property info from listing card
         */
        const extractBasicPropertyInfo = ($, url) => {
            const title = cleanText($('h1, h2, h3').first().text());
            const priceText = cleanText($('[class*="price"]').first().text());
            const locationText = cleanText($('[class*="location"]').first().text());
            
            // Extract bedrooms and bathrooms from icons or text
            let bedrooms = null;
            let bathrooms = null;
            let area = null;

            $('[class*="property-"]').each((_, el) => {
                const text = $(el).text();
                const match = text.match(/(\d+)/);
                
                if (match) {
                    const num = parseInt(match[1]);
                    if (text.toLowerCase().includes('bed')) bedrooms = num;
                    if (text.toLowerCase().includes('bath')) bathrooms = num;
                }
            ...
            const agentName = cleanText($('[class*="agent"]').first().text());
            const postedDate = cleanText($('[class*="listed"], [class*="date"]').first().text());
            ...
            try {
                $('script[type="application/ld+json"]').each((_, script) => {
                    ...
                    for (const item of items) {
                        ...
                        if (type === 'RealEstateListing' || type === 'Product' || type === 'Offer' || type === 'Residence') {
                            return {
                                title: cleanText(item.name || item.title),
                                price: cleanText(item.price || item.offers?.price),
                                location: cleanText(item.address?.addressLocality || item.address?.streetAddress),
                                description: cleanText(item.description),
                                propertyType: cleanText(item.category || item['@type']),
                                bedrooms: item.numberOfRooms || item.numberOfBedrooms || null,
                ...
                log.debug('JSON-LD parsing failed', { error: err.message });
            }
            return null;
        };

        /**
         * Extract data from HTML when JSON-LD is not available
         */
        const extractFromHtml = ($, url) => {
            const title = cleanText($('h1').first().text());
            const price = cleanText($('[class*="price"]').first().text());
            const location = cleanText($('[class*="location"]').first().text());
            const description = cleanText($('[class*="description"]').first().text());
            ...
            $('[class*="detail"], [class*="feature"], [class*="amenity"]').each((_, el) => {
                const text = $(el).text().toLowerCase();
                const match = text.match(/(\d+)/);
                
                if (match) {
                    const num = parseInt(match[1]);
                    if (text.includes('bed')) bedrooms = num;
                    if (text.includes('bath')) bathrooms = num;
                }
                
                if (text.includes('sqft') || text.includes('sq ft')) {
                    area = cleanText($(el).text());
                }
            });

            propertyType = cleanText($('[class*="property-type"]').first().text());
            agentName = cleanText($('[class*="agent"]').first().text());
            postedDate = cleanText($('[class*="listed"], [class*="posted"]').first().text());
        ...
        const crawler = new CheerioCrawler({
            ...
            async requestHandler({ request, $, enqueueLinks, log: crawlerLog }) {
                ...
                if (label === 'LIST') {
                    ...
                    if (collectDetails) {
                        ...
                        // Extract basic info directly from listing page
                        const propertyData = extractBasicPropertyInfo($, request.url);
                        
                        if (propertyData && !seenUrls.has(request.url)) {
                            seenUrls.add(request.url);
                            await Dataset.pushData(propertyData);
                            savedCount++;
                            crawlerLog.info(`Saved property from listing: ${propertyData.title}`);
                        }
                    }
                ...
                if (label === 'DETAIL') {
                    ...
                    try {
                        ...
                        if (propertyData && (propertyData.title || propertyData.price)) {
                            await Dataset.pushData(propertyData);
                            savedCount++;
                            crawlerLog.info(`Saved property: ${propertyData.title} (${savedCount}/${RESULTS_WANTED})`);
                        } else {
                            crawlerLog.warning('No property data extracted from detail page');
                        }
                    } catch (err) {
            ...
            async failedRequestHandler({ request }, error) {
                log.error(`Request ${request.url} failed multiple times`, { error: error.message });
            },
        });
